/*
  "Playback" an array of saved values in an arbitrarty order
...
//were a value is saved to apply to the LED tube. These will be used for brigntness
float seq1[20] = {0, 0, 0, 0, .25, 0, 0, 0, 0, .5, 0, 0, 0, 0, 1, 0, 0, 0, 0, .6};

//The order of the elemnts in the seq1 array we want to show
...
void loop() {
  ...
  //the rate at whic hwe step through the sequence
  if (current_time - prev_time[2] > seq1_rate) {
    prev_time[2] = current_time;
    seq_location++;
    if (seq_location > 6) {
      //once we're done turn all the lights off
      seq_location = 0;
      for (byte m = 0; m < 20; m++) {
  ...
  if (current_time - prev_time[0] > 33) { //33 milliseconds is about 30 Hz, aka 30 fps
    ...
    //the led we want to set is seq1_order[seq_location]
    //seq_location just goes 0-6 but seq1_order[seq_location] will give us the order of lights we've put in that array
    //color is set randomy so we can see which one is currently being addressed
    //the brightness is set by the seq1 array at the location given by seq1_order[seq_location]


    set_LED(seq1_order[seq_location], random(8) / 8.0, 1, seq1[seq1_order[seq_location]]);
    //if seq_location was cahnged to going to 0-19 this would jsut display the seq1 bank from 0-19
    //at the rate of seq1_rate
    //set_LED(seq_location, random(8) / 8.0, 1, seq1[seq_location]);

    //this is the way we've been doing things
    //it sets all the lights at the saem time, without animation.
     //    for (byte m = 0; m < 20; m++) {
    //      set_LED(m, random(8) / 8.0, 1, seq1[seq_location]);
    //    }
