...
unsigned long prev_time[8]; //an array of 8 unsigned long  variables all named "prev_time"
int latch[4];
...
void loop() {
  ...
  if (current_time - prev_time[1] > 33) { //33 milliseconds is about 30 Hz, aka 30 fps
    ...
    for (byte m = 0; m < 20; m++) {
      ...
      //there's a function in this sketch bellow the loop which makes it easier to control the LEDs
